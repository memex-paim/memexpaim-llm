# CLAUDE.md — Memex PAIM LLM
> Pi5 | `/home/admin/server/memexpaim-llm/` (napló/docs)
> Fejlesztési fájl: `/home/admin/server/memex-paim/llm/index.html`

## CC ESZKÖZÖK

**FONTOS — a kód nem itt van!** Fejleszteni ide kell cd-zni:
```bash
cd /home/admin/server/memex-paim
# Fejlesztési fájl: llm/index.html
```

**ripgrep — HTML/JS keresés:**
```bash
cd /home/admin/server/memex-paim
rg "WebLLM\|Qwen\|IndexedDB\|pendingArticle" llm/index.html  # kulcs változók
rg "iro:\|memex-db\|RAG\|FTS"               llm/index.html  # DB és keresés
rg "function \w\+"                           llm/index.html  # függvények listája
rg "version\|v2026"                          llm/version.json  # aktuális verzió
```

**Git push workflow (push = azonnal éles!):**
```bash
cd /home/admin/server/memex-paim
git add llm/index.html llm/version.json
git commit -m "leírás"
git push origin master
# ~1-2 perc → él: memexpaim.com/llm/
```

**Nincs Python, nincs venv, nincs SQLite MCP** — böngésző oldali IndexedDB, nem lekérdezhető CC-vel.
...
| Modell | Státusz |
|--------|---------|
| `Qwen2.5-0.5B-Instruct-q4f32_1-MLC` | ✅ Jelenlegi — angolul pontos, 7-12 tok/s |
| `gemma-3n-E2B-it-q4f32_1-MLC` | ⏳ Következő teszt — Google Gemma 3n, ~2B effektív param |
| `gemma-3n-E4B-it-q4f32_1-MLC` | ⏳ Következő teszt — Google Gemma 3n, ~4B effektív param |
| Qwen3-0.6B | ❌ Elvetva — lassabb, magyarul ugyanolyan rossz |
| SmolLM2-360M | ❌ Elvetva — shader-f16 WebGPU hiba |

**Google Gemma 3n** (Google I/O 2025) — ha a projekt folytatódik, ezzel kell tesztelni:
- MatFormer architektúra, on-device/WebGPU-ra optimalizált
- E2B (~2B) és E4B (~4B) — ugyanaz a súlyfájl, dinamikus méretezés
- WebLLM model ID-kat teszteléskor pontosítani kell (fenti nevek valószínűek)

**Angol only** — kis modellek magyarul használhatatlanok.
